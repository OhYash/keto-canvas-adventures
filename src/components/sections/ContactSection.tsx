...
const ContactSection: React.FC<ContactSectionProps> = ({
  ...
  return (
    <Card
      className={`w-[95vw] sm:w-[90vw] md:w-[680px] max-w-[680px] max-h-[85vh] overflow-y-auto ${gradient} backdrop-blur-sm border-slate-600/50 shadow-xl`}
    >
      <CardHeader className="pb-4">
        ...
        <div className="flex items-center justify-between mb-4">
          <a
            ...
            className="flex items-center gap-1.5 text-slate-700 hover:text-slate-900 transition-colors bg-white/80 hover:bg-white px-3 py-1.5 rounded-lg text-sm font-medium shadow-sm border border-slate-200/50"
          >
          ...
          <button
            ...
            className="p-2 bg-white/80 hover:bg-white text-slate-700 hover:text-slate-900 rounded-lg shadow-sm border border-slate-200/50 transition-all duration-200 cursor-pointer"
            title="Copy page link"
          >
            {icon ? <span className="text-base sm:text-lg">{icon}</span> : <Mail className="w-4 h-4" />}
          </button>
        </div>

        {/* Section Heading */}
        <div className="text-center">
          <HeadingTag className="text-2xl sm:text-3xl font-bold text-slate-900 mb-2 tracking-tight">
            {title}
          </HeadingTag>
          <p className="text-slate-600 sm:text-slate-700 text-sm sm:text-base">
            {subtitle}
          </p>
        </div>
      </CardHeader>
